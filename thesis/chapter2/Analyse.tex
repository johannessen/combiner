...
\footnote{In 2013 wurde ein noch größerer, 20.~Maßstab ergänzt%
% (von null an gezählt \cf{Dijk82}die Zoomstufe 19)
...
Zur Herstellung der Kacheln der Web-Karte wird notwendigerweise für jede \term{Zoomstufe} eine Auswahl der jeweils darzustellenden Objektklassen vorgenommen.
Beispielsweise werden Anliegerstraßen \term{(residentials)} unterhalb von \term{zoom}~12 nicht mehr dargestellt (Abbildungen~\ref{fig:kampen11} und~\ref{fig:kampen12}).
...
\twofigures{t}{
	...
	\caption{Karte von \url{osm.org}, \term{zoom}~11, \term{residentials} nicht dargestellt \citex{map:osm-carto}}
	\label{fig:kampen11}
}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/kampen-z12}
	\caption{Karte von \url{osm.org}, \term{zoom}~12, \term{residentials} dargestellt \citex{map:osm-carto}}
	\label{fig:kampen12}
}
...
\twofigures{t}{
	...
	\caption{Karte von \url{osm.org}, \term{zoom}~14, \term{residentials} $\unit[3]{px}$ breit \citex{map:osm-carto}}
	\label{fig:kampen14}
}{
	\includegraphics[width=7cm]{../chapter2/kampen-z17}
	\caption{Karte von \url{osm.org}, \term{zoom}~17, \term{residentials} $\unit[12]{px}$ breit \citex{map:osm-carto}}
	\label{fig:kampen17}
}
...
\twofigures{t!}{
	...
	\caption{Karte von \url{osm.org}, \term{zoom}~13, \term{residentials} $\unit[3]{px}$ breit \citex{map:osm-carto}}
	\label{fig:kampen13}
}{
}
\capstarttrue
...
Anliegerstraßen haben für die \term{Zoomstufen} 14 bis 19 jeweils eine unterschiedliche Strichbreite von $\unit[3]{px}$ bis $\unit[17]{px}$ definiert, so dass die Darstellung der Straße in der Karte Maßstabsänderungen andeutungsweise folgt (Abbildungen~\ref{fig:kampen17} und~\ref{fig:kampen14}). \cf{All15}
...
% entweder durch Pfeil oder (1) kenntlich machen
Grund dafür sind nicht tatsächliche Unterschiede im Ausbauzustand oder der Verkehrsbedeutung, sondern der Umstand, dass die Fahrbahnen der niederrangigen Straße durch einen breiten, parkähnlichen Grünstreifen getrennt werden  (Abbildung~\ref{fig:utrecht16}).
In kleinerem Maßstab überlappen sich die verbreitert gezeichneten Striche der beiden Fahrbahnen und fließen zusammen. 
...
% entweder Satz weglassen oder durch (2) kenntlich machen

% gera/utrecht - evtl. MapQuest / osm.de hier einbringen
\twofigures{ht}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/utrecht-z14}
	\caption{Karte von \url{osm.org}, \term{zoom}~14, Zusammenfließen von benachbarten Fahrbahnen \citex{map:osm-carto}}
	\label{fig:utrecht14}
}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/utrecht-z16}
	\caption{Karte von \url{osm.org}, \term{zoom}~16, Fahrbahnen getrennt erkennbar \citex{map:osm-carto}}
	\label{fig:utrecht16}
}
...
% TODO: nix "oben" -- "unten" wäre passend => umschreiben
...
% durch (1) kenntlich machen (oder evtl. diesen Teil komplett weg)

\twofigures{ht}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/gera-z14}
	\caption{Karte von \url{osm.org}, \term{zoom}~14, Straße neben Bahnstrecke \citex{map:osm-carto}}
	\label{fig:gera14}
}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/gera-z13}
	\caption{Karte von \url{osm.org}, \term{zoom}~13, Straße teilweise durch Bahnstrecke verdeckt \citex{map:osm-carto}}
	\label{fig:gera13}
}
...
% durch (2)/(3) kenntlich machen

%(Dies könnte durch Verdrängung gelöst werden; siehe dort.)

% Fraglich, ob Platz für beide Beispiele Gera+Utrecht geopfert werden soll. Die Probleme sind ja recht ähnlich. Es zeigt so eigentlich nur, dass es kein Einzelfall ist. Eventuell beide Beispiele nehmen, aber nur auszugsweise beschreiben?
% Eventuell Gera ganz weg -- es geht am Thema "Bewertung" teilweise vorbei
...
\twofigures{ht}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/trollstigen-z11}
	\caption{Trollstigen, Web-Karte mit \osm-Daten, $1\nobreak:\nobreak135\,000$ \citex{map:thunderforest}}
	\label{fig:trollstigen-osm}
}{
	
	\includegraphics[width=7cm]{../chapter2/trollstigen-kartverket-z11}
	...
	\caption{Trollstigen, manuell kartographisch generalisiert, $1\nobreak:\nobreak135\,000$ \citex{map:n250}}
}
...
\onefigure{!b}{
	...
	\caption{Überkreuzen paralleler Linienzüge nach Vereinfachung mit Ramer-Douglas-Peucker \citex[bearbeitet]{map:osm-carto}}
}
% TODO: Grafik neu erstellen: Originalkarte -> PSD -> Image/Adjustments/Black&White + QGIS-Export zur Registrierung => PDF in AI

Eines der Probleme bei der Automatisierung der Formvereinfachung besteht in der Notwendigkeit, alle vereinfachten Linien aufeinander abzustimmen, damit die charakteristischen Formen im Kartenbild insgesamt erhalten bleiben. \cf[Formv.]{BK01}
% TODO: ibid.
...
\onefigure{ht}{
	...
	\caption{typisches Generalisierungsergebnis in Web-Karten; fiktives Beispiel nach [sgk02, 49]}
	\label{fig:verdraengung}
}

\twofigures{H}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/stuelinghausen-me}
	\caption{straßenbegleitender Radweg (rot) schneidet Straßenfläche \citex{map:me}}
	...
	\caption{straßenbegleitender Radweg (blau strichliert) von Straße verdeckt \citex{map:osm-carto}}
...
% TODO explizit: Zu lösen wäre dieser Konflikt durch eine Generalisierung durch Verdrängung.
...
% kann dies in Kap. 6 quantifiziert werden?
...
\twofigures{ht}{
	\includegraphics[width=7cm]{../chapter2/b256@2-z16}
	\caption{variierende Abstände von Richtungsfahrbahnen \citex[bearbeitet]{map:osm-carto}}
	\label{fig:b256}
}{
...
In Deutschland erfasst die \osm-\term{community} autobahnähnliche Straßen als \osmtag{highway=trunk} aufgrund ihres Ausbauzustands.
...
Gleiches gilt u.~a. für Norwegen (\term{riksveg}, Abbildung~\ref{fig:trunk-norway}). \cf{osm:HighwayTrunk}
...
\twofigures{ht}{
	...
	\caption{typischer \osmtag{highway=trunk} in Norwegen (E\,39, Vallaheiane)}
	\label{fig:trunk-norway}
}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/trunk-no-ev39-map1}
	\caption{Landstraße (Vallaheiane) als \osmtag{highway=trunk} mit autobahnähnlicher Signatur \citex{map:osm-de}}
	\label{fig:trunk-norway-osm-de}
}

Wird den Gepflogenheiten in Deutschland entsprechend für \osmtag{highway=trunk} eine auf den Ausbau mit baulich getrennten Fahrbahnen hindeutende Signatur gewählt, so birgt dies im internationalen Einsatz Potenzial für Verwirrung (Abbildung~\ref{fig:trunk-norway-osm-de}).
...
Mit \osmtag{tracks=*} wird in \osm{} die Anzahl der Eisenbahngleise gekennzeichnet, für die ein \term{way} steht.
Abbildungen~\ref{fig:tracks-small} und~\ref{fig:tracks-large} stellen diesen \term{tag} farblich dar.
...
Möglicherweise hat ein \osm-Beitragender \osmtag{tracks=*} missverstanden als die Gesamtanzahl der Gleise \cf{Fox12}, wofür jedoch ein anderer \term{tag} steht (\osmtag{passenger\_lines=*}).

\twofigures{ht}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/tracks-small}
	\caption{Zusammenfließen benachbarter Bahngleise in kleinem Maßstab}
	\label{fig:tracks-small}
}{
	\includegraphics[width=\ScaleIfNeeded]{../chapter2/tracks-large}
	\caption{einzelne Gleise erkennbar in größerem Maßstab}
	\label{fig:tracks-large}
}
% TODO: fiktives Beispiel (?)
% <http://www.itoworld.com/map/14> ca. 2012/2013
...
Der Abzweig (3) und (4) besteht tatsächlich aus jeweils einem einzelnen Gleis, das fälschlich \osmtag{tracks=2} gesetzt hat.

% Fußnote?
Insgesamt ist diese Karte mit \osmtag{tracks=*} wenig gelungen.
Hier zeigt sich eine Schwäche von \osm: Zwar ist mit \term{relations} eine logische Verknüpfung paralleler Gleise in der \osm-Datenbank technisch möglich, jedoch ist dies nicht zuverlässig und simpel genug, dass es von einer Mehrheit der \osm-Beitragenden angewandt wird.
Das unglücklich benannte \osmtag{passenger\_lines} reicht für manche Anwendungen aus, wird jedoch nur auf speziellen Karten angezeigt,
% zu implizit?
...
\onefigure{ht}{
	...
	\caption{Eisenbahnstrecken in Europa, für die \osmtag{passenger\_lines} erfasst ist \citex{map:ito-passenger-lines}}
...
Beim Zusammenfassen können Attribute der beteiligten \term{ways} derart aggregiert werden, dass zuvor durch Geometrie ausgedrückte Zusammenhänge in den generalisierten Daten erhalten bleiben.
Beispielsweise kann beim Zusammenfassen paralleler Gleise die in \osmtag{passenger\_lines} vermerkte Gleisanzahl der zuvor getrennten \term{ways} für das generalisierte Ergebnis addiert werden.
...
Zusammen mit Richardson beschreibt er eine Möglichkeit, die Eigenschaft der Kontinuität von Linienzügen (genannt \term{stroke}) zu nutzen, um deren Wichtigkeit einzustufen und damit ein Straßennetz zu generalisieren („Principle of Good Continuation“). \cf{TR99}
