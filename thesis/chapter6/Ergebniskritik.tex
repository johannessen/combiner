...
Die genutzten Geodaten beschränken sich auf einen Testdatensatz mit dem Straßennetz im Land Nordrhein-Westfalen vom November~2012.
...
% roads-motorway.shp
Der \term{Combiner} erkennt $\unit[4461]{km}$ davon als parallel; das ausgegebene Ergebnis besteht zu $\unit[97,6]{\%}$ aus zusammengefassten Linienzügen.
...
Im NRW-Testdatensatz erkennt der \term{Combiner} $\unit[54]{km}$ Fahrbahn nicht als parallel; wird die Straßennummer nicht berücksichtigt, sinkt diese Länge auf $\unit[31]{km}$.

An mehreren der dann neu erkannten Stellen trägt nur eine der beiden Richtungsfahrbahn die Straßennummer als Attribut.
In Abschnitt~\ref{ch:case-selection} wurde dargelegt, dass im Straßennetz grundsätzlich eine vergleichsweise hohe Datenqualität zu erwarten wäre, weil Fehler darin in der Standard-Karte auf \href{https://www.openstreetmap.org/}{\nolinkurl{osm.org}} störend sichtbar sind und deswegen zügig von \osm-Beitragenden behoben werden.
...
Dieses zusätzliche Verfahren ist standardmäßig aktiv, kann aber mit dem Schalter \texttt{--no-cleanup} kontrolliert werden.
...
Mit diesem Verfahren können jedoch in der gegenwärtigen Implementierung nur solche Topologielücken beseitigt werden, die erst beim Zusammenfassen von Parallelen entstanden sind.
...
\onefigure{t}{
	\begin{overpic}[width=7cm]{../chapter6/breitscheid-bg}
		\put(0,0){\includegraphics[width=7cm]{../chapter6/breitscheid-gen-styled}}
	\end{overpic}
	% 1:50000 Google Mercator, bbox 761000 6682100 764500 6684100
	\caption{Fehlende Verbindungsrampen im Generalisierungsergebnis}
	\label{fig:breitscheid-gen-styled}
}

% offen: relocateGeneralisedNodes löst Problem nicht vollständig: funktioniert nicht (?), wenn Querstraße generalisiert ist? -> eher für 6.3.2
...
Das für ein Straßennetz entwickelte Verfahren zur Beseitigung von Topologielücken aus Abschnitt~\ref{ch:relocateGeneralisedNodes} wird durch die spitzen Winkel der Gleiswechsel verwirrt und führt hier nicht zu einem gelungenen Ergebnis (Abbildung~\ref{fig:rail-kalk-detail}).
% evtl. Ausschnittsvergrößerung
...
Auffallend ist ferner, dass für Bahnstrecken in Köln mehr als 20~Segmente als „nah“ beurteilt werden.
...
Durch bessere Anpassung der Definition von \textproc{Parallel} aus Abschnitt~\ref{ch:analyse-algorithm} auf die Bedingungen im Eisenbahnnetz (z.~B. $\unit[15]{m}$ \textproc{Distanz} und $8\degree$ Ausrichtungsunterschied) lässt sich dies geringfügig verbessern.
