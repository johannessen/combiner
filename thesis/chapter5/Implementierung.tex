...
Für 4.3.4 ergibt sich folgendes Datenmodell/Klassenstruktur:
\onefigure{ht}{
...
% TODO: 4.3.4 spricht hier von Segmenten, die aneinandergehangen werden. Ist auch so implementiert, über AbstractLine, welches eine Liste von Segmenten ist. Aber macht das so wirklich Sinn? Die Segmente braucht doch keiner mehr. Eventuell nur eine Liste von Punkten speichern und dann lazy initialisation? Dafür könnte evtl. recht einfach ResultLine einige Methoden überschreiben (coordinates, add (-> protected), get/size/iterator etc.) oder wir bauen gleich AbstractLine so um, dass es lazy ist (weil ja ohnehin auch vom SourceWay her eine Node-Liste reinkommt).
