...
Es seien nun $n = |S|$ die Anzahl der Segmente in den Eingangsdaten, $\nu$ die durchschnittliche Anzahl der jeweils „nahen“ Segmente und $\phi$ die durchschnittliche Anzahl der Punktezuordnungen je Punkt.
...
\\
\textproc{Zusammenfassen} &\in \mathcal{O}(n \cdot \phi)
\end{align*}

Jedoch sind $\nu$ und $\phi$ im Verhältnis zu $n$ vernachlässigbar, weil sie regelmäßig sehr klein sind.
