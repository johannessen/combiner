...
Daher werden die \term{segments} für die Analyse auf Parallelität solange immer weiter unterteilt, bis schließlich ein einfacher geometrischer Vergleich möglich ist (Abbildung~\ref{fig:comparable-fragments}).
...
\State $\textbf{Anfangswert } wurzel(s) \stackrel{\text{\tiny def}}{=} s$ $\forall$ $s$
...
%
Die Unterscheidung der Seiten links und rechts kann durch Bestimmung der Winkel zu den \term{nodes} des jeweils anderen Segments erfolgen.
...
\State $\textbf{Anfangswert } parallelLinks(s) \stackrel{\text{\tiny def}}{=} \varnothing$ $\forall$ $s$
\State $\textbf{Variable } parallelRechts : Segment \rightarrow Segment$
\State $\textbf{Anfangswert } parallelRechts(s) \stackrel{\text{\tiny def}}{=} \varnothing$ $\forall$ $s$
...
Dabei kommen nur solche Segmente für die Kennzeichnung in Frage, die im Kontext der jeweiligen Anwendung als \textproc{Parallel} gelten.
...
%	\State \textbf{Ergebnis} $||\textproc{Mittelpunkt}(s_1) - \textproc{Mittelpunkt}(s_2)||$
	\State \textbf{Ergebnis} $||\textproc{Start}(s_1) - \textproc{Start}(s_2) + \textproc{Ende}(s_1) - \textproc{Ende}(s_2)|| \cdot \frac{1}{2}$
\EndFunction
\end{algorithmic}
\end{algorithm}
...
Der Algorithmus \textproc{NodesZuordnen} sucht für jeden \term{node} beidseitig den jeweils am nächsten gelegenen \term{node} der als parallel gekennzeichneten Segmente.
