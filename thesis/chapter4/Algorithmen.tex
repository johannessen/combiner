...
% TODO: Lingo erklären: alle Fragmente sind neue Segmente, also Verbindung zw. zwei Punkten
...
\Function{Splitten}{$S$}
	...
	\ForAll{$b \textbf{ mit } b \in B$}
		\ForAll{$n \textbf{ mit } n \in \{\textproc{Start}(b), \textproc{Ende}(b)\} $}
			\ForAll{$t \textbf{ mit } t \in \textproc{NaheSegmente}(b, S) \wedge \neg$ $gesplittet(t) \wedge \exists$ $p_F : p_F = \textproc{Fußpunkt}(n, t)$}
				\State $\{f_1, f_2\} \gets \{(\textproc{Start}(t), p_F), (p_F, \textproc{Ende}(t))\}$
...
Aus Gründen der Effizienz bietet es sich an, diese \textproc{Hülle} rechteckig im Koordinatensystem anzulegen.
Folge daraus ist, dass die Größe des Puffers von der Orientierung des Segments abhängt.
...
Für die Korrektheit der Algorithmen spielt dies jedoch keine Rolle.
% weil?
% TODO: Grafik?
...
\begin{algorithmic}
\Function{Hülle}{$s$}
	\State \textbf{Ergebnis} sei die nach allen Seiten um $\mu/2$ vergrößerte rechteckige Hülle um das Segment $s$, wobei $\mu$ der festzulegende Höchstabstand sei, für den zwei Segmente oder Fragmente als „nahe“ gelten dürfen.
\EndFunction
\end{algorithmic}
\end{algorithm}
...
%	\State Es sei $l \bot t$ und $n \in l$. Dann sei $f \coloneq l \cap t$ bei $min(||f-t_1||, ||f-t_2||) > \varepsilon$ $\wedge$ $max(||f-t_1||, ||f-t_2||) < ||t||$, wobei $\varepsilon$ die festzulegende Mindestlänge eines Fragments sei.
	\State \textbf{Ergebnis} sei der Lotfußpunkt~$f$ des Punkts~$n$ auf der Geraden~$t$. Liegt~$f$ nicht zwischen den Punkten $\textproc{Start}(t)$ und $\textproc{Ende}(t)$ oder liegt~$f$ näher an $\textproc{Start}(t)$ oder $\textproc{Ende}(t)$ als die festzulegende Mindestlänge~$m$ eines Fragments, dann gibt es kein Ergebnis.
...
$\textbf{Variable } parallelLinks : Segment \rightarrow Segment$
\\$\textbf{Anfangswert } parallelLinks(a) \stackrel{\text{\tiny def}}{=} \varnothing$ $\forall$ $a$
\\$\textbf{Variable } parallelRechts : Segment \rightarrow Segment$
\\$\textbf{Anfangswert } parallelRechts(a) \stackrel{\text{\tiny def}}{=} \varnothing$ $\forall$ $a$
\begin{algorithmic}
\Function{Analyse}{$S$}
...
\begin{algorithm}[H]
\caption{Distanz TBD}\label{alg:Distanz}
\begin{algorithmic}
