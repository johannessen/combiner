	...
	\ForAll{$b := (n_1, n_2) \textbf{ mit } b \in B $}%\Comment{Segment $b = (n_1, n_2)$}
		\ForAll{$n \textbf{ mit } n \in \{n_1, n_2\} $}
			\ForAll{$t \textbf{ mit } t \in \textproc{NaheSegmente}(b, S) \wedge \neg$ $gesplittet(t) \wedge \exists$ $p_F : p_F = \textproc{Fußpunkt}(n, t)$}
				\State $t =: (n_3, n_4)$
				\State $\{f_1, f_2\} \gets \{(n_3, p_F), (p_F, n_4)\}$
...
\Function{Analyse}{$S$}
	\ForAll{$s \textbf{ mit } s \in S, s =: (n_1, n_2)$}
		...
		\State $L \gets \{t : t \in T, t =: (n_3, n_4) \wedge \sphericalangle (s, n_3 - n_1) < 0 \wedge \sphericalangle (s, n_4 - n_1) < 0\}$
		\State $R \gets \{t : t \in T, t =: (n_3, n_4) \wedge \sphericalangle (s, n_3 - n_1) > 0 \wedge \sphericalangle (s, n_4 - n_1) > 0\}$
	...
	\ForAll{$s \textbf{ mit } s \in S, s =: (n_1, n_2)$}% \wedge \neg$ $fertig7(t)$}
		\ForAll{$t \textbf{ mit } t \in \{parallelLinks(s), parallelRechts(s)\}, t =: (n_3, n_4)$}
			\ForAll{$e_1 \textbf{ mit } e_1 \in \{n_1, n_2\} $}
				\State $e_2 \gets \begin{cases}n_3 & \text{für } ||e_1 - n_3|| < ||e_1 - n_4||\\n_4 & \text{sonst}\end{cases}$
